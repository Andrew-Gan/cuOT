...
#include "emp-ot/ferret/tree_exp.h"
...
public:
	...
	void compute(blk *ggm_tree_mem, blk secret) {
		this->delta = secret;
		gpu_ggm_tree_send(lSum, rSum, ggm_tree_mem, secret_sum_f2, secret, depth);
	}
	...
	void send_f2k(OT * ot, IO * io2, int s) {
		...
		cudaMemcpy(lSum_cpu, lSum.data(), (depth-1)*sizeof(blk), cudaMemcpyDeviceToHost);
		cudaMemcpy(rSum_cpu, rSum.data(), (depth-1)*sizeof(blk), cudaMemcpyDeviceToHost);
