...
void FerretCOT<T>::setup(std::string pre_file) {
	...
	if(hasfile & hasfile2) {
		...
		Delta = (blk)read_pre_data128_from_file((void*)buffer, pre_ot_filename);
		cudaMemcpy(ot_pre_data.data(), buffer, param.n_pre*sizeof(blk), cudaMemcpyHostToDevice);
		delete[] buffer;
	} else {
...
int64_t FerretCOT<T>::rcot_inplace(vec &ot_buffer, int64_t byte_space) {
	...
	for(int64_t i = 0; i < round; ++i) {
		if(party == ALICE) {
			cudaMemcpy(tmp, ot_pre_data, M*sizeof(block), cudaMemcpyDeviceToHost);
		    pre_ot->send_pre(tmp, Delta);
		}
		else {
			pre_ot->recv_pre(tmp);
			cudaMemcpy(ot_pre_data, tmp, M*sizeof(block), cudaMemcpyHostToDevice);
		}
		extend(ot_buffer, mpcot, pre_ot, lpn_f2, ot_pre_data);
		// pt += ot_limit;
		// memcpy(ot_pre_data, ot_buffer + ot_limit, M*sizeof(block));
		cudaMemcpy(ot_pre_data, ot_buffer.data(ot_limit*(i+1)), M*sizeof(block), cudaMemcpyDeviceToDevice);
	}
...
void FerretCOT<T>::assemble_state(void * data, int64_t size) {
	...
	cudaMemcpy(array + sizeof(int64_t) * 4 + sizeof(block), ot_pre_data.data(), sizeof(block)*param.n_pre, cudaMemcpyDeviceToHost);
	// if (ot_pre_data!= nullptr)
	// 	delete[] ot_pre_data;
	// ot_pre_data = nullptr;
	ot_pre_data.clear();
...
int FerretCOT<T>::disassemble_state(const void * data, int64_t size) {
	...
	cudaMemcpy(ot_pre_data.data(), array + sizeof(int64_t) * 4 + sizeof(block), sizeof(block)*param.n_pre, cudaMemcpyHostToDevice);
