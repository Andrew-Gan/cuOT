...
template<typename T>
class FerretOTSender : public FerretOT<T> {
public:
  ...
  virtual ~FerretOTSender() {
    ...
    if (ot_pre_data.size() > 0) {
      this->write_pre_data_to_file((void*)tmp, (__uint128_t)Delta, pre_ot_filename);
    }
    delete[] tmp;
    delete base_cot;
...
private:
  virtual void setup(std::string *pre_file) {
    if(pre_file != nullptr) pre_ot_filename = *pre_file;
      ...
      pre_ot_filename=PRE_OT_DATA_REG_SEND_FILE;
    }
  ...
  virtual void seed_expand(Span &ot_output, MpcotReg<T> *mpcot, OTPre<T> *preot, Mat &ot_input) {
    ...
    block *tmp = new block[mConfig.lpnParam->n_pre];
    memcpy_D2H_dev(tmp, ot_pre_data.data(), M*sizeof(blk));
