...
yvals = [[
        [109, 25, 19, 14, 5],
        [214.65, 36.58, 26.72, 20.22, 8.28],
        [401.45, 59.57, 39.99, 30.66, 15.55]
    ], [
...
xlabels = [
    ['CPU', 'NAIVE GPU', 'IMPROVED_AES', 'COALESCED_TREE_EXP', 'COALESCED_NODE_SUM'],
    ['CPU', 'CUFFT_POLYMUL', 'IMPRV_BIT_TRANSP', 'IMPRV_CMPLX_PROD', 'TYPE_CAST_AND_MOD'],
    ['CPU', 'BIT_SHIFT_MATMUL', 'E2E_PRIMAL_LPN'],
]
# hr = [6, 12]
fontSize='xx-large'
...
for xlab, t, ys, f in zip(xlabels, titles, yvals, filenames):
    ...
    for i, y in enumerate(ys):
        y = list(map(int, y))
        xloc = [i + j * 0.18 for j in range(len(y))]
        bars = plt.bar(xloc, y, width=0.1, color=[f'C{i}' for i in range(len(y))])
        plt.legend(bars, xlab, loc='upper left', fontsize='x-large', ncol=5)

        for i, t in zip(xloc, y):
            plt.annotate(f'{t}', xy=(i, t), ha='center', va='bottom', fontsize=fontSize)

        xvals += xloc
    plt.xticks([i+0.075*len(y) for i in range(0, len(ys))], labels=['2²³', '2²⁴', '2²⁵'], fontsize=fontSize)
    plt.xlabel('Number of OT', fontsize=fontSize)
