...
public:
  ...
  T* data(uint64_t r, uint64_t c) const { return (T*)mPtr + (r * mRows + c); }
  void set(uint64_t r, uint64_t c, T &val);
  void resize(uint64_t r, uint64_t c);
  void bit_transpose();
  void modp(uint64_t reducedCol);
  void xor_scalar_async(T *rhs, cudaStream_t s);
...
void GPUmatrix<T>::modp(uint64_t reducedCol) {
  ...
  for (uint64_t i = 0; i < mCols / reducedCol; i++) {
    gpu_xor<<<grid, block>>>(mPtr, mPtr + (i * reducedCol * sizeof(T)), mCols);
    cudaDeviceSynchronize();
  }
  mCols = reducedCol;
}
...
template<typename T>
void GPUmatrix<T>::xor_scalar_async(T *rhs, cudaStream_t s) {
  uint64_t nBlock = (mNBytes + 1023) / 1024;
  gpu_xor_single<<<nBlock, 1024, 0, s>>>(mPtr, (uint8_t*) rhs, sizeof(T), mNBytes);
}

template<typename T>
GPUmatrix<T>& GPUmatrix<T>::operator&=(T *rhs) {
  uint64_t nBlock = (mNBytes + 1023) / 1024;
  gpu_and_single<<<nBlock, 1024>>>(mPtr, (uint8_t*) rhs, sizeof(T), mNBytes);
  cudaDeviceSynchronize();
}
