...
static void cipher(state_t* state, const uint8_t* roundKey) {
  ...
  // These NUM_ROUNDS rounds are executed in the loop below.
  ...
  add_round_key(NUM_ROUNDS, state, roundKey);
}
...
static void inv_cipher(state_t* state, const uint8_t* roundKey) {
  ...
  for (round = (NUM_ROUNDS - 1); ; --round) {
    ...
    inv_mix_columns(state); // takes a lot more than than mix_columns()
  }
}
...
static void* _encryptWorker(void *args) {
  ...
  for(int i = tArgs.start; i < tArgs.end; i++) {
    cipher((state_t*)&tArgs.buf->content[AES_BLOCKLEN * i], tArgs.ctx->roundKey);
  }
  return NULL;
}

void aes_ecb_encrypt(AES_ctx* ctx, AES_buffer* buf, int numThread) {
...
static void* _decryptWorker(void *args) {
  ...
  for(size_t i = tArgs.start; i < tArgs.end; i++) {
    inv_cipher((state_t*)&tArgs.buf->content[AES_BLOCKLEN * i], tArgs.ctx->roundKey);
  }
  return NULL;
}

void aes_ecb_decrypt(AES_ctx* ctx, AES_buffer* buf, int numThread) {
  size_t workload = buf->length / AES_BLOCKLEN / numThread;
  int rem = (buf->length / AES_BLOCKLEN) % numThread;
  pthread_t *t = (pthread_t*) malloc((numThread + 1) * sizeof(*t));
  ThreadArgs *args = (ThreadArgs*) malloc((numThread + 1) * sizeof(*args));
