...
TreeNode *prf;
TreeNode *otNodes;
volatile bool *volatile otSent = nullptr;

static void xor_prf(TreeNode *sum, TreeNode *prf, TreeNode *pprf, int start, int end) {
...
void pprf_sender_cpu(TreeNode *root, size_t depth,
    ...
    for(int t = 0; t < numTrees; t++) {
        ...
        for (int i = 0; i < numThread; i++) {
            ...
            accumulator.push_back(std::thread(xor_prf, sumLeaves, prf, nullptr, start, end));
        }
        for (int i = 0; i < numThread; i++) {
            accumulator.at(i).join();
        }
        // printf("Sender cpu expansion %d complete\n", t);
    }
...
void pprf_recver_cpu(void (*initialiser)(AES_ctx*, const uint8_t*),
    ...
    for(int t = 0; t < numTrees; t++) {
        ...
        for (int i = 0; i < numThread; i++) {
            int start = i * workload;
            int end = start + workload - 1;
            accumulator.push_back(std::thread(xor_prf, sparseVec, prf, pprf, start, end));
        }
        for (int i = 0; i < numThread; i++) {
            accumulator.at(i).join();
        }
        // printf("cpu recv sparse: ");
        // for(int i = 0; i < numLeaves; i++) {
        //   printf("%x ", sparseVec[i].data[0]);
        // }
        // printf("\n");
        nonZeroRows[t] = puncturedIndex;
        // printf("Recver cpu expansion %d complete\n", t);
    }
